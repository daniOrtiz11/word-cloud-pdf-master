...
function getHtml(fileName) {
	var filePath = "fichas_docentes/" + fileName;
	...
	extract(filePath, { splitPages: false }, function (err, text) {
	  if (err)
		...
		fs.writeFile("nubes_ficha_docente/" + fileName.slice(0,fileName.lastIndexOf('.pdf')) + ".html", str1 + JSON.stringify(allWords.slice(0,cloud_size)) + str2, function (err) {
		  if (err)
			throw err;
		});
	  }
...
function getFiles() {
	var fs = require('fs');
	fs.readdir("fichas_docentes", function(err, files) {
		if (err)
			throw err;
		for (var file in files)
			if (files[file].lastIndexOf('.pdf') == files[file].length - 4)
				getHtml(files[file]);
	});
}
